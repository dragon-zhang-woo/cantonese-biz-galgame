...
- 自定义情境支持输入 20–500 字真实难题，在浏览器先隐私脱敏，再生成
  2–3 轮有边界的训练；API 不可用时自动匹配最近的预置场景；
...
结果。自定义情境编排器只从受控模板、技能卡和来源库组合训练轮次；模型
不可以创建任意故事节点或修改故事图。
...
2. 展示本地隐私处理回执、技能卡、依据来源和 2–3 轮训练；
3. 完成训练，展示六维行为量表、港式改写和可迁移行动模板；
4. 返回训练手册，筛选十二项正式场景；

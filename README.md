...
- 七个可独立练习的真实工作任务：任务澄清、优先级协商、坏消息沟通、
  非正式建立关系、柔性跟进、范围控制和高层汇报；
- 四名主要 NPC 在主线与训练库中各有至少三次可玩互动，不再只是一次性角色；
- 每项训练先说明任务与隐藏关系风险，结束后给出双模型反馈、港式改写和
  可直接带回现实工作的复用句式；
...
结果。若模型误把原问题当成新反应重复，语义防线会拒绝该结果并安全
降级。模型不可以创建节点或修改故事图。
...
1. 从主菜单打开“7 个情境训练”，选择“先确认，再开工”；
2. 展示本轮任务与隐藏关系风险，再进入现场；
3. 自己输入一句商务回应，展示双模型角色反应、港式改写、任务得分和
   可迁移句式；
4. 返回训练库，展示四名 NPC 的七项真实工作任务与本机完成记录；
5. 切回五幕标准剧情，证明网络故障不会破坏主线 Demo。
...
- 应用不会持久化玩家的自由作答；AI 即兴模式会把当前一轮文本发送
